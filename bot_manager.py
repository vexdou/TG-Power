...
class DynamicBotManager:
    ...
    async def start_bot_instance(self, bot_id: int, token: str) -> bool:
        ...
        try:
            ...
            logger.info(f"🤖 Authenticated Bot: @{bot_me.username} ({bot_id})")

            # 3. Clear Webhook to allow Polling
            await handler.app.bot.delete_webhook(drop_pending_updates=True)
            
            # 4. Start Application and Updater
            ...
            logger.info(f"🟢 Managed Bot @{bot_me.username} is fully ONLINE!")
            return True
    ...
    async def stop_bot_instance(self, bot_id: int):
        if bot_id in self.running_bots:
            try:
                ...
                logger.info(f"🛑 Managed Bot {bot_id} stopped.")
            except Exception as e:
