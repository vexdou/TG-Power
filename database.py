...
class Database:
    ...
    async def init_db(self):
        try:
            ...
            await self.users.create_index([("bot_id", 1), ("user_id", 1)], unique=True)
            logger.info("✅ MongoDB Indexes initialized successfully.")
        except Exception as e:
            logger.error(f"❌ Database initialization error: {e}")

    # --- MANAGED BOTS OPERATIONS ---
    async def add_new_bot(self, owner_id: int, token: str, bot_id: int, username: str = ""):
        bot_data = {
            "bot_id": bot_id,
            "owner_id": owner_id,
            "token": token,
    ...
    async def save_bot_user(self, bot_id: int, user_id: int, username: str, full_name: str):
        user_data = {
            "bot_id": bot_id,
            "user_id": user_id,
            "username": username,
        ...
        await self.users.update_one(
            ...
            {"$setOnInsert": {"language": Config.DEFAULT_LANG}, "$set": user_data},
            upsert=True
        )
    ...
    async def set_user_language(self, bot_id: int, user_id: int, lang: str):
        await self.users.update_one(
            {"bot_id": bot_id, "user_id": user_id},
            {"$set": {"language": lang}}
        )
    ...
    async def get_all_bot_users(self, bot_id: int):
        cursor = self.users.find({"bot_id": bot_id})
        return await cursor.to_list(length=None)
    ...
    async def log_download(self, bot_id: int, user_id: int, platform: str, media_type: str):
        ...
        await self.downloads.insert_one(record)

    async def get_bot_stats(self, bot_id: int):
        total_users = await self.users.count_documents({"bot_id": bot_id})
